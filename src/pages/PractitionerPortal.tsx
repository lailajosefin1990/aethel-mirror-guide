...
import { usePractitionerClients, type Client } from "@/hooks/usePractitionerClients";
...
const PractitionerReadingOutput = ({
  reading, domain, clientName, practitionerName, onSave, onBack,
}: {
  reading: ReadingData; domain: string; clientName: string;
  practitionerName: string; onSave: () => void; onBack: () => void;
}) => {
  ...
  const handlePrint = () => { track("practitioner_pdf_exported"); window.print(); };
    ...
    <>
      <style>{`
        @media print {
          ...
          #practitioner-print { position: absolute; left: 0; top: 0; width: 100%; background: white !important; color: black !important; padding: 40px; }
          ...
          #practitioner-print p, #practitioner-print span, #practitioner-print h2 { color: black !important; }
      ...
      <div id="practitioner-print" ref={printRef}>
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="font-body text-[11px] uppercase tracking-[0.2em] text-primary mb-10">{domain}</motion.p>
        <div className="mb-10">
        ...
        <div className="border-t-2 border-primary/40 pt-8 mb-10">
          ...
          <p className="font-display text-[22px] sm:text-[24px] leading-[1.4] text-foreground text-center font-medium">{reading.third_way}</p>
        </div>
        ...
        <div className="flex flex-col gap-3 pb-10 no-print">
          ...
          <button onClick={handlePrint} className="w-full h-[48px] rounded-sm border border-primary text-primary font-body text-[14px] hover:bg-primary/10 transition-all duration-300 flex items-center justify-center gap-2">
...
const PractitionerPortal = () => {
  ...
  const { practitioner, clients, loading, maxClients, addClient, updateClient, deleteClient } = usePractitionerClients(user);
  const sessions = usePractitionerSessions(practitioner);
  ...
  const getFormData = () => ({
    name: formName, email: formEmail, birthDate: formBirthDate,
    birthTime: formBirthTime, birthTimeUnknown: formBirthTimeUnknown,
    birthPlace: formBirthPlace, notes: formNotes,
  });

  const resetForm = () => {
    setFormName(""); setFormEmail(""); setFormBirthDate("");
    setFormBirthTime(""); setFormBirthTimeUnknown(false);
    setFormBirthPlace(""); setFormNotes("");
  };

  const handleAddClient = async () => {
    const result = await addClient(getFormData());
    if (result === "limit") { setLimitModalOpen(true); return; }
    if (result) { resetForm(); setView("clients"); }
  };

  const handleUpdateClient = async () => {
    if (!selectedClient) return;
    const ok = await updateClient(selectedClient.id, getFormData());
    if (ok) setView("client_detail");
  };
  ...
  const openClientDetail = async (client: Client) => {
    ...
    await sessions.loadClientReadings(client.id);
  ...
  const startReading = () => {
    sessions.resetReading();
    setView("new_reading");
  };

  const generateReading = useCallback(async () => {
    if (!selectedClient) throw new Error("No client selected");
    await sessions.generateReading(selectedClient);
  }, [selectedClient, sessions]);

  const handleSaveReading = async () => {
    if (!selectedClient) return;
    await sessions.saveReading(selectedClient.id);
    ...
    return (
      <section className="min-h-screen px-5 py-8">
        <div className="w-full max-w-app mx-auto text-center pt-16">
          ...
          <button onClick={handlePractitionerCheckout} className="w-full h-[52px] rounded-sm bg-primary text-primary-foreground font-body font-medium text-[14px] tracking-wide hover:brightness-110 transition-all duration-300">
            Start Practitioner — £49/month
          </button>
          <button onClick={() => navigate("/")} className="w-full font-body text-[13px] text-foreground/50 hover:text-foreground/70 transition-colors mt-4">
            Maybe later
          </button>
        </div>
      </section>
        ...
        <AnimatePresence mode="wait">
          ...
          {view === "clients" && (
            <motion.div key="clients" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="flex items-center justify-between mb-8">
                ...
                <p className="font-display text-[14px] tracking-[0.35em] text-primary">Y O U R &nbsp; C L I E N T S</p>
                <span className="font-body text-[12px] text-muted-foreground">{clients.length}/{maxClients}</span>
              </div>
              {clients.length === 0 ? (
                  ...
                  {clients.map((client) => (
                    <button key={client.id} onClick={() => openClientDetail(client)}
                      ...
                      <div className="text-left">
                        ...
                        {client.birth_place && <p className="font-body text-[12px] text-muted-foreground">{client.birth_place}</p>}
                      </div>
              ...
              <button onClick={() => { if (clients.length >= maxClients) { setLimitModalOpen(true); return; } resetForm(); setView("add_client"); }}
          ...
          {(view === "add_client" || view === "edit_client") && (
            <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <button onClick={() => view === "edit_client" && selectedClient ? openClientDetail(selectedClient) : setView("clients")}
              ...
              <div className="space-y-4 mb-8">
                ...
                <div>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={formBirthTimeUnknown}
                      onChange={(e) => { setFormBirthTimeUnknown(e.target.checked); if (e.target.checked) setFormBirthTime(""); }}
              ...
              <button onClick={view === "add_client" ? handleAddClient : handleUpdateClient}
            ...
            <motion.div key="detail" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  {selectedClient.birth_place && <p className="font-body text-[12px] text-muted-foreground">{selectedClient.birth_place}</p>}
                </div>
                <button onClick={() => openEditClient(selectedClient)} className="font-body text-[12px] text-primary hover:text-primary/80 transition-colors">Edit</button>
              </div>
              ...
              <p className={`${sectionLabel} mb-4`}>R E A D I N G S &nbsp; ({sessions.clientReadings.length})</p>
              {sessions.clientReadings.length > 0 ? (
                <div className="space-y-3 mb-6">
                  {sessions.clientReadings.map((r) => {
                    const isExpanded = sessions.expandedReadingId === r.id;
                    const rd = r.reading_json as any;
                    return (
                      <div key={r.id} className="bg-card border border-border rounded-md p-4">
                        <button onClick={() => sessions.setExpandedReadingId(isExpanded ? null : r.id)} className="w-full text-left">
                          ...
                          {!isExpanded && <p className="font-body text-[12px] text-muted-foreground italic">{rd?.third_way?.slice(0, 80)}...</p>}
              ...
              <div className="space-y-4 mb-8">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {sessions.domains.map((d) => (
                      <button key={d} onClick={() => sessions.setReadingDomain(d)}
                        className={`px-3 py-2.5 rounded-sm border font-body text-[13px] transition-all duration-200 ${
                          sessions.readingDomain === d ? "border-primary bg-primary/10 text-primary" : "border-border text-foreground/60 hover:border-foreground/30"
                        }`}>
                        {d}
                      </button>
                    ))}
                ...
                <div>
                  ...
                  <textarea value={sessions.readingQuestion} onChange={(e) => sessions.setReadingQuestion(e.target.value)}
                    placeholder="What is the client sitting with?" rows={3} className={`${inputClass} resize-none`} />
                </div>
              </div>
              <button onClick={() => setView("loading")} disabled={!sessions.readingQuestion.trim()}
                className="w-full h-[52px] rounded-sm bg-primary text-primary-foreground font-body font-medium text-[14px] tracking-wide hover:brightness-110 transition-all duration-300 disabled:opacity-50">
                Find my Third Way →
              </button>
            </motion.div>
          ...
          {/* ─── Reading Result ─── */}
          {view === "reading_result" && sessions.readingData && selectedClient && practitioner && (
            <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <PractitionerReadingOutput
                reading={sessions.readingData}
                domain={sessions.readingDomain}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <p className="font-body text-[14px] text-muted-foreground leading-relaxed mb-6">
                ...
                <a href="mailto:hello@aethelmirror.com" className="text-primary hover:underline">hello@aethelmirror.com</a>{" "}
                to discuss an expanded plan.
              </p>
              <button onClick={() => setLimitModalOpen(false)}
